...
def Pareto(DATA_FRAME,column_index,column_values,color="#9B59B6"):
    ...
    figure1=figure(x_range=DATA_FRAME[column_index],title=column_values, plot_width=600,plot_height=450,
    ...
    #Create the Graphs
    figure1.vbar(x=column_index,top=column_values,width=0.9,fill_color=color,line_color="#2E4053",source=DataSourse)
    figure1.line(x=column_index,y='Porcentaje_acumulado',y_range_name='y2',source=DataSourse)
    figure1.scatter(x=column_index,y='Porcentaje_acumulado',y_range_name='y2',source=DataSourse)
    
    #Create the hover for can show the values of graph
    hover=HoverTool(tooltips=[(column_index,'@'+column_values),('% profit acumulete','@Porcentaje_acumulado'), 
                              ('% profit','@Porcentaje')])
...
def barra_stackeada(DATA_FRAME,column_index,column_columns,column_values):
    ...
    figure1=figure(x_range=sellers,height=250,title=column_values,plot_width=1200,plot_height=800,
